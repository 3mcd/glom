...
describe("transformer", () => {
  ...
  test("transforms Out query", () => {
    ...
    expect(output).toContain("Object.defineProperty(on_removed, \"__system_desc\"")
  ...
  test("transforms multiple queries in one system", () => {
    ...
    expect(output).toContain("params: [{ all: [{ write: Position }] }, { all: [{ read: Velocity }] }]")
  })
  ...
  test("transforms Rel join query", () => {
    ...
    expect(output).toContain("params: [{ all: [{ write: Position }, { rel: [TargetOf, { read: Position }] }] }]")
  })
})
