...
import {advanceTimestep, makeTimestep, timestepSetOffset} from "./timestep"
...
describe("timestep", () => {
  test("basic fixed update accumulation", () => {
    ...
    advanceTimestep(ts, 0, () => ticks++)
    expect(ticks).toBe(0)

    advanceTimestep(ts, 33.4, () => ticks++)
    expect(ticks).toBe(2)
  ...
  test("reset on large time skip (maxDrift)", () => {
    ...
    advanceTimestep(ts, 1000, () => ticks++)

    expect(ticks).toBe(0)
    expect(ts.accumulated).toBe(0)

    advanceTimestep(ts, 1017, () => ticks++)
    expect(ticks).toBe(1)
  })
