...
describe("join query", () => {
  ...
  test("Join with relation", () => {
    ...
    const system = g.defineSystem(
      (
        query: g.Join<g.All<typeof Position>, g.All<typeof Name>, typeof ChildOf>,
      ) => {
        for (const [pos, name] of query) {
          results.push([pos, name])
        }
  ...
  test("Reactive Join (partial In)", () => {
    ...
    const system = g.defineSystem(
      (
        query: g.Join<g.In<typeof Position>, g.All<typeof Name>, typeof ChildOf>,
      ) => {
        for (const [pos, name] of query) {
          results.push([pos, name])
        }
  ...
  test("Reactive Join (partial Out)", () => {
    ...
    const system = g.defineSystem(
      (
        query: g.Join<g.Out<typeof Position>, g.All<typeof Name>, typeof ChildOf>,
      ) => {
        for (const [pos, name] of query) {
          results.push([pos, name])
        }
