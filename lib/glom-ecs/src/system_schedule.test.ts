...
describe("system_schedule sorting", () => {
  ...
  test("writer before reader", () => {
    ...
    const system_read = () => { order.push("read") }
    define_system(system_read, { params: [{ read: A }] })

    const system_write = () => { order.push("write") }
    define_system(system_write, { params: [{ write: A }] })
    ...
    run_system_schedule(schedule, world as any)
  ...
  test("multiple writers in registration order", () => {
    ...
    const w1 = () => { order.push(1) }
    define_system(w1, { params: [{ write: A }] })

    const w2 = () => { order.push(2) }
    define_system(w2, { params: [{ write: A }] })
  ...
  test("complex chain", () => {
    ...
    const s3 = () => { order.push("S3") }
    define_system(s3, { params: [{ read: B }] })

    const s2 = () => { order.push("S2") }
    define_system(s2, { params: [{ read: A }, { write: B }] })

    const s1 = () => { order.push("S1") }
    define_system(s1, { params: [{ write: A }] })
  ...
  test("cycle detection", () => {
    const s1 = () => {}
    define_system(s1, { params: [{ write: A }, { read: B }] })

    const s2 = () => {}
    define_system(s2, { params: [{ write: B }, { read: A }] })
    ...
    const world = make_world()
    expect(() => run_system_schedule(schedule, world as any)).toThrow("Cycle detected in system dependencies")
  })
  ...
  test("independent systems use registration order", () => {
    ...
    define_system(s1, { params: [{ read: A }] })

    const s2 = () => { order.push(2) }
    define_system(s2, { params: [{ read: B }] })
